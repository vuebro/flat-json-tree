...
let value: null | Reactive<Record<string, unknown>[]> = null;
    ...
    if (parent) {
      ...
      if (parent[keyParent]) {
        siblings.splice(
          ...
          0,
          ...((parent[keyChildren] ?? []) as Record<string, unknown>[]).splice(
...
function useFlatJsonTree(
  ...
  ({
    branch: keyBranch = keyBranch,
    children: keyChildren = keyChildren,
    id: keyId = keyId,
    index: keyIndex = keyIndex,
    next: keyNext = keyNext,
    parent: keyParent = keyParent,
    prev: keyPrev = keyPrev,
    siblings: keySiblings = keySiblings,
  } = keys);
